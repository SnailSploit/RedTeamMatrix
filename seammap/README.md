...
**The complete catalogue of new techniques** the model surfaced lives in
[`docs/NEW-TECHNIQUES.md`](docs/NEW-TECHNIQUES.md) (generated by `gen-new-techniques.cjs`): all 114
AGENT-DISCOVERED seams across 10 discovery rounds, each with its trust assumption, validity verdict, MITRE
status (40 carry no ATT&CK id), and falsifiable validation. Full per-technique walkthroughs (attack mechanics +
runnable PoC) are in [`docs/WALKTHROUGHS.md`](docs/WALKTHROUGHS.md). Each is backed by a web-verified technical
note (`data/tech-notes.json`, joined at load): all 114 carry a concrete exploitation mechanism, an explicit
**MITRE-gap statement** (the closest ATT&CK/ATLAS id and *why* it is insufficient), and real-world anchors
(269 in total — CVEs and named incidents) — enforced by `src/validate.ts` so none can regress to a stub.

**Measured against the full catalogue.** `data/mitre-attack.json` is the complete MITRE ATT&CK Enterprise
